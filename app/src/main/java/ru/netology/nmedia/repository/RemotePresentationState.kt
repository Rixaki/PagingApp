...
fun Flow<CombinedLoadStates>.asRemotePresentationState(): Flow<RemotePresentationState> =
    scan(RemotePresentationState.INITIAL) { state, loadState ->
        when (state) {
            ...
            RemotePresentationState.APPEND -> when (loadState.source.append) {
                is LoadState.Loading -> RemotePresentationState.APPEND
                else -> state
            }
            RemotePresentationState.PREPEND -> when (loadState.source.prepend) {
                is LoadState.Loading -> RemotePresentationState.PREPEND
                else -> state
            }
